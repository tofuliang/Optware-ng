...
#C_ARES_CONFFILES=$(TARGET_PREFIX)/etc/c-ares.conf $(TARGET_PREFIX)/etc/init.d/SXXc-ares
...
#
# Binaries should be installed into $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/sbin or $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/bin
# (use the location in a well-known Linux distro as a guide for choosing sbin or bin).
# Libraries and include files should be installed into $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/{lib,include}
# Configuration files should be installed in $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/c-ares/...
# Documentation files should be installed in $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/doc/c-ares/...
# Daemon startup scripts should be installed in $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/S??c-ares
#
...
#	$(INSTALL) -d $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/
#	$(INSTALL) -m 644 $(C_ARES_SOURCE_DIR)/c-ares.conf $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/c-ares.conf
#	$(INSTALL) -d $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d
#	$(INSTALL) -m 755 $(C_ARES_SOURCE_DIR)/rc.c-ares $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXc-ares
#	sed -i -e '/^#!/aOPTWARE_TARGET=${OPTWARE_TARGET}' $(C_ARES_IPK_DIR)$(TARGET_PREFIX)/etc/init.d/SXXc-ares
